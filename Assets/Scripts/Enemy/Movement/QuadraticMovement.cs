...
public class QuadraticMovement : MonoBehaviour, IEnemyMovement
{
    ...
    private void Update()
    {
        if (!isFrozen)
        {
            transform.position = Lerp.QuadraticBezier(startingPoint.position, control0.position, endPoint.position, speed * Time.deltaTime);
        }
